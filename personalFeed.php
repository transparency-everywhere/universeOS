...
?>
    ...
    <script>
        if($("#friendRequest_<?=$friendRequestData['owner'];?>").length == '0'){
            $('#dockMenuBuddyAlerts').append('<li id="friendRequest_<?=$friendRequestData['owner'];?>"><?=showUserPicture("$friendRequestData2[userid]", '15', '', true);?><div class="messageMain"><a href="#" onclick="showProfile(\'<?=$friendRequestData2[userid];?>\');"><?=$friendRequestData2[username];?></a> wants to be your friend</div><div class="messageButton"><a href="doit.php?action=requestpositive&buddy=<?=$friendRequestData2[userid];?>" target="submitter" class="btn btn-info btn-mini" style="margin-right:25px;" onclick="$(\'#friendRequest_<?=$newGroupData[id];?>\').remove();">Add to Buddylist</a><a href="doit.php?action=requestnegative&buddy=<?=$friendRequestData2[userid];?>" class="btn btn-mini" target="submitter" onclick="$(\'#friendRequest_<?=$friendRequestData2[id];?>\').remove();">Decline</a></div></li>');
        }
    </script>
<?
$i++;
