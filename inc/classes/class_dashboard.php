...
class dashBoard{
	...
	function showTaskBox($grid=true){
		...
		foreach($taskArray AS $task){
			...
			$output .= '<li><a href="#" onclick="tasks.show('.$task['id'].','.$editable.');">'.shorten(date('d.m.', $task['timestamp']).' - '.$task['title'],14).'</a><span onclick="tasks.markAsDone('.$task['id'].'); return false" class="icon white-check"></span><span onclick="tasks.verifyRemoval('.$task['id'].'); return false" class="icon white-close"></span></li>';
		}
		$output .= '</ul>';
