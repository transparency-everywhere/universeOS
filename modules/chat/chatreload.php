 ...
 ?>
      <div class="chatMainFrame">
          <header class="grayBar">
              ...
              <span><a href="javascript: toggleKey('<?=$buddy;?>');" id="toggleKey_<?=$buddy;?>"><i class="lockIcon"></i></a></span>
          ...
          <div id="chatKeySettings_<?=$buddy;?>" class="chatKeySettings">
          </div>
...
<? } ?>          
          ...
          <div class="chatMainFrame_<?=$buddyName;?>">
		      ...
		      unset($intWindows);?>
              <div onclick="chatLoadMore('<?=$buddy;?>', '1');">...load more</div>
          </div>
          <?
          if(empty($_GET[initter])){ ?>
      ...
      <footer class="blackGradient">
          <center style="margin-top: 6px;">
              <form action="doit.php?action=chatSendMessage&buddy=<?=$buddy;?>&buddyname=<?=$buddyName;?>" method="post" target="submitter"  autocomplete="off" onsubmit="chatMessageSubmit('<?=$buddy;?>');">
				  ...
				  <input type="hidden" name="cryption" value="false" id="chatCryptionMarker_<?=$buddy;?>">
